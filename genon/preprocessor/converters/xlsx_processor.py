...
def _detect_header(
    ...
    while i < n:
        ...
        if hms:  # 수평 병합 있음 → 계층 헤더행
            ...
            i += 1
            continue
        # 병합 없는 행: '컬럼의 과반이 채워진' 행을 컬럼명행(leaf)으로 본다.
        ...
        # 헤더로 승격되어 한 행이 유실된다. 반대로 '빈 칸 N개 이하' 같은 절대 기준은
        # 표가 넓어질수록 정상 헤더(예: 15열 중 3칸 빔)를 배너로 오판한다.
        # 그래서 열 수에 비례하는 '과반' 기준을 쓴다: 채워진 칸이 전체 컬럼의 과반(절반 초과)이면 헤더.
        #   - '□ 제조업 등'(9칸 중 1칸), 좁은 표의 2칸 제목행(4칸 중 2칸=절반)은 과반 미달 → 스킵
        #   - 15칸 중 12칸 채운 헤더는 과반 → 헤더 유지(데이터행 승격 방지)
        # (2열 이하 표는 이 기준이 불안정하므로 기존 동작 유지: 병합 없는 첫 행 = leaf.)
        if len(used_cols) >= 3 and len(ne) * 2 <= len(used_cols):
            title_rows.append(i)  # 과반 미달(대부분 빈 배너/제목행) → 스킵하고 다음 행 탐색
            i += 1
            continue
        leaf_idx = i  # 병합 없는 첫 '과반이 채워진' 행 → 컬럼명행
        break
