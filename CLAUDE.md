...
`genon/preprocessor/facade/legacy/` 는 **기본적으로 참조하지 않는다** — 별도 배포 단위이므로 활성 경로 작업에 끌어들이지 말고, 사용자가 명시적으로 언급할 때만 읽는다.
...
`_ext_aliases` 를 임베디드 intel 사본에서 가져오지 않은 **한 줄**이었다. 그런데 조사 중 발견한
곁가지까지 함께 고쳐 205줄(docling 포함 6파일 + 신규 모듈 2개)이 됐다. 되돌린 것과 그 이유:

| 넣었던 것 | 왜 과했나 |
|---|---|
| 확장자 미등록 시 내용으로 포맷 판정(74줄) | 설정 별칭이 이미 하는 일. 게다가 `- item` 으로 시작하는 yaml/평문이 md 백엔드로 잘못 가 본문 한 줄이 소실됐다(실측) |
| `get_pdf_path` 반환 규칙 변경 | 원본 파괴를 **다른 파괴로 옮겼다**(파생 PDF 가 원천 디렉터리에 잔류, 동명 문서 덮어쓰기). 고칠 자리는 쓰는 쪽이었다 |
| docling `html_backend.get_text` (40줄) | 표기 차이일 뿐 내용 유실이 아니었고, 영향 범위가 모든 HTML 표 파싱 |
| artifacts 경로 공용 모듈 + 파사드 5곳 배선(70줄) | 필요한 것은 파사드 한 곳의 3줄이었다 |
...
- **docling 은 되도록 수정하지 않는다.** 포크 본체를 건드리면 영향 범위가 그 백엔드를 쓰는
  모든 문서로 퍼지고 배포에 wheel 재빌드가 강제된다(핫픽스 overlay 는 genon 전용).
  **그 docling 결함을 고치는 것이 이번 작업의 목표일 때만** 손댄다. 조사 중 우연히 발견한
  docling 결함은 별도 이슈로 분리하고, 지금 작업에는 넣지 않는다.
  - 고쳐야 할 때가 되면 **genon 우회책이 아니라 docling 안에서** 고친다 — 우회책을 기본
    해법으로 삼으면 나중에 진짜 수정을 가로챈다. 백엔드가 고쳐지면 그 자리를 메우던
    genon 우회책은 걷어낸다.
