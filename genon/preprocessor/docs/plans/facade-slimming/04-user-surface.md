...
> **상태: 완료 (2026-09-06).** 골든 56케이스 차이 0, 유닛 전체 실패 집합 동일(40건).
>
> - **조정 지점 블록** — `chunking_processor.py` 는 머리에 모으고 끝에 경계를 그었다.
>   `parser_processor.py` 는 상수가 아니라 **고칠 자리 3개**(`ROUTES` / `_route_*` /
>   `_load_json_payload`)를 머리 주석에 적었다. 이 파일의 조정 대상은 값이 아니라 위치다.
> - **`__call__` 라우팅** — 계획대로 `{확장자: 핸들러}` dict 가 아니라 **순서 있는
>   `(확장자, 핸들러)` 목록 + None 폴스루**로 갔다. 폴스루 2곳(.json 미매칭, .md text 모드)이
>   그대로 살아 있다. 덤으로 docling 분기 5곳이 복붙하던 마무리 6줄이 `_docling_response`
>   하나로 모였다.
> - **`_load_json_payload` 시그니처** — `doc_type` 을 받는다. 06 드릴이 이 게이팅 덕에
>   골든 차이 0 으로 통과했다.
> - **설정 프로필** — 새 키는 넣지 않고 두 간소 프로필에 "무엇을 위한 판이고 어디를 보면
>   되는지" 머리말만 붙였다.
> - **문서** — 개발 매뉴얼 5장의 줄번호를 전부 걷어냈고(대조해 보니 파일 지목까지 틀려
>   있었다), `gitbook_doc/chunking_processor.md` 를 신설했으며, 낡은 `facade/README.md` 를
>   지웠다. `installation.md` 의 상충 메시지도 정리했다.
