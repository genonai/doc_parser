...
class AudioLoader:
    def __init__(self, file_path: str, req_url: str, req_data: dict, chunk_sec: int = 29, tmp_path: str = "."):
        self.file_path = file_path
...
class IntelligentDocumentProcessor:
    ...
    def __init__(self, config: dict | None = None):
        ...
        try:
            ...
            _log.warning(f"[IntelligentDocumentProcessor] Invalid page_batch_size '{page_batch_size}', fallback to 32")
            page_batch_size = 32
    ...
    def _build_ocr_options(ocr_cfg: dict, paddle_endpoint: str):
        ...
        if ocr_engine not in {"paddle", "upstage"}:
            _log.warning(f"[IntelligentDocumentProcessor] Unknown ocr.engine '{ocr_engine}', fallback to 'paddle'")
            ocr_engine = "paddle"
        ...
        try:
            ...
            for table_idx, table_item in enumerate(document.tables):
                ...
                for cell_idx, cell in enumerate(table_item.data.table_cells):
                    ...
                    cell_bbox = _fitz.Rect(bbox.l, min(bbox.t, bbox.b), bbox.r, max(bbox.t, bbox.b))
...
class DocxDocumentLoader:
    ...
    def __init__(self):
        ...
        self.converter = DocumentConverter(
            format_options={
                InputFormat.DOCX: WordFormatOption(pipeline_cls=SimplePipeline, backend=GenosMsWordDocumentBackend),
            }
        )
...
class DocumentProcessor:
    ...
    def __init__(self, config_path: str | None = None):
        ...
        self._whisper_req_data = {
            ...
            "response_format": whisper_cfg.get("response_format", attach_cfg.get("whisper_response_format", "json")),
        ...
        try:
            self._whisper_chunk_sec = int(whisper_cfg.get("chunk_sec", attach_cfg.get("whisper_chunk_sec", 29)))
        except (TypeError, ValueError):
    ...
    def _parse_docling(self, file_path: str, **kwargs) -> DoclingDocument:
        ...
        if ocr_mode == "force":
            ...
            if ocr_mode == "auto":
                if not check_document(document, self._intel.enrichment_options) or self._intel.check_glyphs(document):
                    document = self._intel.load_documents_with_docling_ocr(file_path, **kwargs)
    ...
    def _parse_hwp_hwpx(self, file_path: str, **kwargs) -> DoclingDocument:
        ...
        try:
            ...
            if ext in (".hwp", ".hwpx"):
                try:
                    return self._hwp.load_documents(file_path, **dict(kwargs, use_hwp_sdk=False))
                except Exception:
    ...
    def setup_logging(self, level_num: int):
        ...
        logging.basicConfig(
            level=level, format="%(asctime)s [%(levelname)s] %(name)s: %(message)s", handlers=[logging.StreamHandler()]
        )
