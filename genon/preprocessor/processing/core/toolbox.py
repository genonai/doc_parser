...
새 구현은 없다 — 고객이 모듈 배치를 몰라도 쓸 수 있게 하는 것이 목적이다.
    ...
    from processing.core import toolbox as tb
...
from genon.preprocessor.processing.enrichment.field_transforms import register_transform

# ── 라우트 산출 조립 ────────────────────────────────────────────────────────
# ROUTES 에 자기 route_* 를 만들었을 때 응답 element 를 만드는 데 쓴다.
from genon.preprocessor.processing.serialize.parse_format import make_elements
...
# ── 청크 텍스트 정제 ────────────────────────────────────────────────────────
from genon.preprocessor.processing.chunking.text_norm import (
...
def set_chunk_metadata(result: dict, metadata: dict) -> dict:
    ...
    """
    from genon.preprocessor.processing.enrichment.field_transforms import store_metadata_in_document
...
from genon.preprocessor.processing.common.appendix import check_appendix_keywords
from genon.preprocessor.processing.common.vector_meta import (
    drop_fields,
    merge_small_chunks,
    split_chunk,
)
...
from genon.preprocessor.processing.common.file_probe import (
    is_encrypted_pdf,
    is_protected_hwp,
...
def refresh_stats(vectors, reindex: bool = True):
    ...
    """
    from genon.preprocessor.processing.common.vector_meta import refresh_stats as _refresh
