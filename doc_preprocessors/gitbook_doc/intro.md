...
*복잡한 표를 정확하게 구조화된 데이터로 변환 - 병합 셀, 계층적 헤더를 JSON으로 구조화*
...
💡 **포인트**: 각 청크가 "인사규정 > 제2장 채용 > 2.1 채용절차" 같은 경로 정보를 포함해서 문맥 유지
...
- 표를 "엔드포인트, 메소드 = /user/login, POST" 형식으로 변환
...
2. **표는 절대 분할 안 함**: 표 데이터 무결성을 위해 4,096 토큰까지 허용
3. **컨텍스트 보존**: 모든 청크에 상위 섹션 경로 포함
