# 적재용(지능형) 전처리기 v.2.2.3 (2026-07-16 Release)
...
def _handle_stage_error(exc: Exception, stage: str) -> None:
    ...
    if _cache_current_context().error_policy == "strict":
        raise GenosServiceException("1", f"[{stage}] {exc}", stage=stage, error_type=error_type) from exc
...
class GenosSmartChunker(BaseChunker):
    ...
    def _split_document_by_tokens(self, doc_chunk: DocChunk, dl_doc: DoclingDocument, **kwargs) -> list[DocChunk]:
        ...
        def split_items_evenly_by_tokens(item_token_counts, max_tokens):
            ...
            if k > n:
                _log.warning(
                    f"[split_items_evenly_by_tokens] n={n} k={k} total={total} max_tokens={max_tokens} (n<k, 빈 구간 발생 가능)"
                )
            else:
                _log.info(f"[split_items_evenly_by_tokens] n={n} k={k} total={total} max_tokens={max_tokens}")
            target = total / k
...
class DocumentProcessor:
    ...
    def __init__(self, config_path: str | None = None):
        ...
        if self._chunk_mode not in {"split_only", "resize_all"}:
            _log.warning(
                f"[DocumentProcessor] Unknown chunking.chunk_mode '{self._chunk_mode}', fallback to 'split_only'."
            )
        ...
        self._guardrail_masking_enabled = bool(
            _parse_optional_bool(gm_cfg.get("masking_enabled"), "guardrail.masking_enabled")
        )
        ...
        if self.image_description_options.chart_enabled:
            try:
                ...
                _log.warning(f"[DocumentProcessor] do_picture_classification 설정 실패: {exc}")
        ...
        self.image_description_enricher = ImageDescriptionEnricher(self.image_description_options)
        self.table_description_enricher = TableDescriptionEnricher(self.table_description_options)
    ...
    def _normalize_runtime_kwargs(self, kwargs: dict) -> dict:
        ...
        normalized["chart_detection"] = _as_int_flag(normalized.get("chart_detection"), detection_default)
        ...
        table_default = _as_int_flag(runtime.get("table_desc"), 1 if (tbase and tbase.enabled) else 0)
        refine_default = _as_int_flag(runtime.get("table_refine"), 1 if (tbase and tbase.refine_enabled) else 0)
    ...
    def _configure_runtime_image_mode(self, kwargs: dict):
        ...
        if base is not None:
            ...
            self.image_description_options = resolve_runtime_image_options(
                ...
                classification_available=getattr(self.pipe_line_options, "do_picture_classification", False),
            )
            self.image_description_enricher = ImageDescriptionEnricher(self.image_description_options)
            _log.info(
        ...
        if dbase is not None:
            self.doc_summary_options = resolve_runtime_doc_summary_options(dbase, doc_summary=doc_summary)
            self.doc_summary_enricher = DocSummaryEnricher(self.doc_summary_options)
            _log.info(
    ...
    async def _process_pdf(
        self, request: Request, file_path: str, converted_pdf_path: Optional[str], is_ppt: bool = False, **kwargs: dict
    ):
        ...
        **kwargs: dict,
        ...
        if gr.call_enabled(kwargs):
            sensitive_infos = gr.classify_document(
                gr.doc_text(document),
                self._guardrail_url,
                self._guardrail_workflow_id,
                self._guardrail_api_key,
                self._guardrail_timeout,
            )
    ...
    async def __call__(self, request: Request, file_path: str, **kwargs: dict):
        ...
        try:
            ...
            return await self._process_pdf(request, file_path, converted_pdf_path, is_ppt=is_ppt, **kwargs)
        finally:
            _log_cache_summary()
