...
> **상태: 완료 (2026-09-06).** `examples/parse_chunk/parse_chunk_golden.py` +
> `golden_normalize.py` 로 구현했다. 케이스 56건 / 산출물 112개, 노이즈 기준선 0.
> 정규화 대상은 실측대로 `reg_date` 하나다.
>
> 계획과 달라진 것 3가지.
> - `--noise` 의 두 회차가 **같은 캐시 스코프**를 쓴다. 나누면 2회차가 LLM 을 다시 불러
>   그 답의 흔들림까지 노이즈로 잡힌다(실측 58개 산출물이 어긋났다).
> - PDF·pptx 는 이 머신에서 **엔드포인트에 닿는다**(layout http 400 응답 확인). 계획이
>   "불가" 로 적은 것은 틀렸다. 다만 pptx 는 PPT→PDF 변환본이 실행마다 달라져 산출이
>   흔들리므로 기본 목록에서 뺐다. PDF 는 그대로 골든에 있다.
> - 머신 종속/비결정 케이스는 `UNSTABLE_CASES` 로 분리하고 **뺐다는 사실을 매 실행 출력**한다.
