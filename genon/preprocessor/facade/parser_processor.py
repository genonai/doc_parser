...
class DocumentProcessor(ParserCore):
    ...
    # 다르게 하려면 아래 표에 적는다. 키는 설정 파일 경로를 dot notation 으로 쓰거나 같은 뜻의
    # 요청 파라미터 이름으로 쓴다. 둘 다 같게 동작한다.
    #
    # 파서에서 쓰는 주요 키. 켜기는 1(또는 True), 끄기는 0 이다.
    ...
    #
    # 요청마다 바꿀 수 없는 설정(엔드포인트 주소, 프롬프트 등)은 건너뛰고 경고를 남긴다.
