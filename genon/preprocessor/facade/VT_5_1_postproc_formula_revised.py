...
# NOTE: 실제 키는 env var (OPENROUTER_API_KEY / TABLE_REFINER_BEARER_TOKEN) 로 주입.
# 운영본 facade 에는 평문이 있으나 본 revised 사본은 시크릿 마스킹 후 커밋함.
DEFAULT_OPENROUTER_API_KEY = "<MASKED>"
DEFAULT_TABLE_REFINER_BEARER_TOKEN = "<MASKED>"
...
def _block_match_score(block_norm: str, hwp_norm: str) -> float:
    """$$ 블록 내용이 hwp 수식에 얼마나 포함되는지 — '블록 길이' 기준 커버리지.

    기존 _formula_coverage_score 는 hwp 길이로 나눠서, B^{(1)} 같은 짧은 수식이
    괄호 몇 글자 겹치는 것만으로 가짜 고득점을 받는 문제가 있었다.
    여기서는 블록(=교체 대상) 길이로 정규화하여, '이 블록을 가장 잘 설명하는 hwp 수식'을 고른다.
    또 hwp 수식이 블록보다 현저히 짧으면(정보량이 더 적으면) 감점한다.
    """
    if not block_norm or not hwp_norm:
        return 0.0
    sm = SequenceMatcher(None, block_norm, hwp_norm)
    matched = sum(x.size for x in sm.get_matching_blocks())
    score = matched / max(len(block_norm), 1)
    if len(hwp_norm) < len(block_norm) * 0.5:
        score *= 0.3
    return score


def _clean_hwp_latex(text: str) -> str:
    """SDK가 내려준 latex 의 명백한 결함만 최소 보정.
    - value 안에 박힌 raw newline/탭 → 공백
    - 알려진 SDK 오타: \\bcd → \\cdot
    수식 구조 자체는 건드리지 않는다 (SDK 출력 신뢰)."""
    t = (text or "").replace("\r", " ").replace("\n", " ").replace("\t", " ")
    t = t.replace("\\bcd", "\\cdot")
...
def _enrich_vectors_with_hwp_formulas(
    ...
    """벡터 텍스트의 $$...$$ 수식 블록을 HWP SDK 수식으로 교체.

    이전 구현은 '벡터 1개 ↔ 수식 1개' DP 매칭이라, 한 청크에 수식이 여러 개 있으면
    대부분 누락되고 엉뚱한 작은 수식이 본문 한복판에 삽입되어 오히려 깨졌다.
    → 매칭 단위를 '벡터'가 아니라 '$$ 블록' 으로 바꿔서, 한 청크 안의 모든 수식 블록이
      각각 올바른 HWP 수식과 1:1 로 순서보존 매칭되도록 한다.
    bbox/페이지 메타는 보존하고 text/n_char 만 갱신한다.
    """
    hwp_formulas = _collect_hwp_formulas(hwp_doc)
    _log.info(f"[수식 후처리] HWP 수식 {len(hwp_formulas)}개 수집")
    for _i, _f in enumerate(hwp_formulas):
        _log.debug(f"[수식 후처리]   hwp[{_i}] p{_f.page_no} text={_f.text!r}")
    if not hwp_formulas:
        _log.info("[수식 후처리] HWP 수식 없음 → 원본 벡터 반환")
        return vectors

    # 1) 모든 벡터의 $$...$$ 블록을 문서 순서대로 수집
    ...
    _log.info(f"[수식 후처리] $$ 블록 {len(blocks)}개 추출 (vectors={len(vectors)})")
    if not blocks:
        _log.info("[수식 후처리] $$ 블록 없음 → 후처리 스킵 (원본 반환)")
        return vectors

    # 2) blocks ↔ hwp_formulas 순서보존 DP 매칭 (블록 단위, 블록기준 커버리지 스코어)
    _BLOCK_MATCH_MIN = 0.40  # 블록 내용의 40% 이상이 hwp 수식에 포함돼야 후보
    candidate_matches = []
    for b_idx, blk in enumerate(blocks):
        norm = _normalize_text(blk["inner"])
        candidates = []
        for h_idx, hwp in enumerate(hwp_formulas):
            if not hwp.norm:
                continue
            score = _block_match_score(norm, hwp.norm)
            if score >= _BLOCK_MATCH_MIN:
                candidates.append((h_idx, score, hwp))
        candidate_matches.append((b_idx, candidates))
    matches = _dp_utils._select_best_toc_text_matching(candidate_matches)
    _log.info(f"[수식 후처리] 블록 단위 DP 매칭 → {len(matches)}개")

    # 3) 매칭을 v_idx 별로 그룹핑
    by_vec: Dict[int, List[Tuple[Dict[str, Any], Any]]] = defaultdict(list)
    for m in matches:
        if m["score"] < enrich_threshold:
            _log.debug(f"[수식 후처리]   SKIP b_idx={m['toc_idx']} score={m['score']:.3f} (<{enrich_threshold})")
            continue
        b_idx = m["toc_idx"]
        h_idx = m["text_idx"]
        blk = blocks[b_idx]
        hwp = hwp_formulas[h_idx]
        by_vec[blk["v_idx"]].append((blk, hwp))
        _log.debug(
            f"[수식 후처리]   MATCH block#{b_idx}(v{blk['v_idx']}) ← hwp#{h_idx} "
            f"score={m['score']:.3f} | inner={blk['inner'].strip()[:50]!r} → hwp={hwp.text!r}"
        )

    # 4) 각 벡터 텍스트에서 뒤쪽 블록부터 치환 (앞쪽 offset 보존)
    vectors = list(vectors)
    applied = 0
    for v_idx, items in by_vec.items():
        vec = vectors[v_idx]
        text = vec.text or ""
        items.sort(key=lambda x: x[0]["start"], reverse=True)
        for blk, hwp in items:
            replacement = f"$$\n{_clean_hwp_latex(hwp.text)}\n$$"
            text = text[:blk["start"]] + replacement + text[blk["end"]:]
            applied += 1
        vectors[v_idx] = vec.model_copy(update={"text": text, "n_char": len(text)})

    _log.info(f"[수식 후처리] 적용 완료: {applied}/{len(blocks)} 블록 교체, 벡터 {len(vectors)}개")
    return vectors
