...
class HybridChunker(BaseChunker):
    ...
    def _split_document_by_tokens(self, doc_chunk: DocChunk, dl_doc: DoclingDocument) -> list[DocChunk]:
        ...
        header_short_info_list = getattr(doc_chunk, '_header_short_info_list', [])  # 짧은 헤더 정보 리스트
        ...
        for i in range(len(items)):
            ...
            if test_tokens <= self.max_tokens:
                ...
                if len(current_items) > 0:
                    ...
                    used_headers = self._extract_used_headers(current_header_short_infos)
                    result_chunks.append(DocChunk(
                        text=chunk_text,
                        meta=DocMeta(
                            doc_items=current_items.copy(),
                    ...
                    used_headers = self._extract_used_headers([header_short_info])
                    result_chunks.append(DocChunk(
                        text=single_text,
                        meta=DocMeta(
                            doc_items=[items[i]],
            ...
            if chunk_tokens < min_chunk_size:
                if current_merge_candidate is None:
                    ...
                    merged_header_short_infos = (
                        getattr(current_merge_candidate, '_header_short_info_list', []) +
                        getattr(chunk, '_header_short_info_list', [])
                    )
                    ...
                    if merged_tokens <= self.max_tokens:
                        current_merge_candidate = DocChunk(
                            ...
                            meta=DocMeta(
                                ...
                                headings=self._extract_used_headers(merged_header_short_infos),
                                captions=None,
                                origin=chunk.meta.origin,
                            )
                        )
                        current_merge_candidate._header_info_list = merged_header_infos
                        current_merge_candidate._header_short_info_list = merged_header_short_infos
                    else:
                        merged_chunks.append(current_merge_candidate)
                        current_merge_candidate = chunk
            else:
...
class DocumentProcessor:
    ...
    def __init__(self):
        ...
        self.enrichment_options = DataEnrichmentOptions(
            ...
            toc_api_base_url="http://llmops-gateway-api-service:8080/serving/68/47/v1/chat/completions",
            metadata_api_base_url="http://llmops-gateway-api-service:8080/serving/68/47/v1/chat/completions",
