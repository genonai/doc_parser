...
async def mlops_exception_handler(request, exc: GenosServiceException):
    ...
    return JSONResponse(
        {"code": exc.error_code, "errMsg": exc.error_msg, "data": None, "error_code": exc.error_code}, status_code=200
    )
...
@app.post("/run")
async def run(request: Request, file_path: str = Body(..., embed=True), params: dict = Body(default_factory=dict)):
    pt = time.time()
    try:
        ...
        return JSONResponse(
            {"code": 1, "errMsg": e.error_msg, "data": None, "error_code": e.error_code, "error_msg": e.error_msg},
            status_code=200,
        )
...
@app.post("/parser")
async def parse(request: Request, file_path: str = Body(..., embed=True), params: dict = Body(default_factory=dict)):
