...
class HybridChunker(BaseChunker):
    ...
    def _generate_text_from_items_with_headers(
        ...
        for i, item in enumerate(items):
            ...
            if isinstance(item, TableItem):
                ...
                # 타이틀과 섹션 헤더 처리 개선
                is_section_header = isinstance(item, SectionHeaderItem) or (
                    isinstance(item, TextItem)
    ...
    def _extract_table_text(
        self, table_item: TableItem, dl_doc: DoclingDocument
    ) -> str:
    ...
    def _split_document_by_tokens(
        self, doc_chunk: DocChunk, dl_doc: DoclingDocument
    ) -> list[DocChunk]:
        """문서를 토큰 제한에 맞게 분할 (여러 섹션이 하나의 청크에 포함 가능)"""
        items = doc_chunk.meta.doc_items
        header_info_list = getattr(
            doc_chunk, "_header_info_list", []
        )  # 각 아이템의 헤더 정보 리스트
    ...
    def _merge_small_chunks(
        self, chunks: list[DocChunk], dl_doc: DoclingDocument
    ) -> list[DocChunk]:
        ...
        min_chunk_size = (
            self.max_tokens // 3
        )  # 최소 청크 크기를 더 크게 설정 (2000/3 = 666토큰)
        merged_chunks = []
        ...
        for i, chunk in enumerate(chunks):
            ...
            if chunk_tokens < min_chunk_size:
                if current_merge_candidate is None:
                    ...
                    # 병합 시도
                    merged_items = (
                        current_merge_candidate.meta.doc_items + chunk.meta.doc_items
                    )
                    merged_header_infos = getattr(
                        current_merge_candidate, "_header_info_list", []
                    ...
                    if merged_tokens <= self.max_tokens:
                        current_merge_candidate = DocChunk(
                            ...
                            meta=DocMeta(
                                ...
                                headings=self._extract_used_headers(
                                    merged_header_infos
                                ),
                                captions=None,
                ...
                if current_merge_candidate:
                    ...
                    if candidate_tokens < min_chunk_size:
                        # 현재 청크와 병합 시도
                        merged_items = (
                            current_merge_candidate.meta.doc_items
...
class GenOSVectorMetaBuilder:
    ...
    def set_chunk_bboxes(
        self, doc_items: list, document: DoclingDocument
    ) -> "GenOSVectorMetaBuilder":
        chunk_bboxes = []
...
class DocumentProcessor:
    ...
    def __init__(self):
        ...
        self.converter = DocumentConverter(
            format_options={
                InputFormat.PDF: VLMLayoutPdfFormatOption(
        ...
        self.second_converter = DocumentConverter(
            format_options={
                InputFormat.PDF: VLMLayoutPdfFormatOption(
                    pipeline_options=pipe_line_options, backend=PyPdfiumDocumentBackend
                ),
            },
        )
    ...
    def split_documents(
        ...
        chunker: HybridChunker = HybridChunker(max_tokens=2000, merge_peers=True)
        ...
        # 문자열인 경우 (쉼표로 구분된 이름들)
            ...
            for sep in separators:
                if sep in authors_data:
                    names = [
                        name.strip() for name in authors_data.split(sep) if name.strip()
                    ]
    ...
    async def __call__(self, request: Request, file_path: str, **kwargs: dict):
        ...
        if "debug_DoclingDocument" in kwargs and kwargs["debug_DoclingDocument"]:
            json_doc = json.loads(document.json())

            for text in json_doc["texts"]:
                page = text["prov"][0]["page_no"]
                size = document.pages.get(page).size

                bbox_data = {
                    "l": text["prov"][0]["bbox"]["l"] / size.width,
                    "t": text["prov"][0]["bbox"]["t"] / size.height,
                    "r": text["prov"][0]["bbox"]["r"] / size.width,
                    "b": text["prov"][0]["bbox"]["b"] / size.height,
                    "coord_origin": text["prov"][0]["bbox"]["coord_origin"],
                }

                text["prov"][0]["bbox"] = bbox_data

            for text in json_doc["pictures"]:
                page = text["prov"][0]["page_no"]
                size = document.pages.get(page).size
            ...
            with open("debug_DoclingDocument.json", "w", encoding="utf-8") as f:
    ...
    def draw_bbox(self):
        ...
        # 입력 경로
        result_json = Path(
            "/workspace/doc_parser/doc_preprocessors/debug_DoclingDocument.json"
        )
        pdf_folder = Path(
            "/workspace/dots_ocr_test/images/한국은행 정부유가증권 사무취급절차(20250318)_일부개정"
        )

        output_path = "./results_doclingdoc"
        ...
        bboxes = (
            [
                {
                    "page": d["prov"][0]["page_no"],
                    "bbox": d["prov"][0]["bbox"],
                    "coord_origin": d["prov"][0]["bbox"]["coord_origin"],
                    "type": d["label"],
                }
                for d in data["texts"]
            ]
