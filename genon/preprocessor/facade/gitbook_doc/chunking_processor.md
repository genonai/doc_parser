...
1. `action: report` 로 배포합니다. 청크는 그대로 적재되고 판정만 로그에 남습니다.
...
한 줄(입력·판정·제외·생존 건수, 사유별 건수, 판정률, 청크가 모두 걸린 페이지)입니다. 본문은 남기지
않습니다.
...
|---|---|---|---|
| 일부 청크 불량 | 전부 반환, 로그 기록 | 해당 청크 제외 후 반환 | 문서 실패 `CHUNK_REJECTED` |
| 전부 불량 | 전부 반환, 로그 기록 | 문서 실패 `CHUNK_ALL_REJECTED` | 문서 실패 `CHUNK_REJECTED` |
