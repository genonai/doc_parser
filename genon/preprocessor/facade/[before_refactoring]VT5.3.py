...
def _convert_to_pdf_sdk(file_path: str) -> str | None:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            _log.info(
                f"[convert_to_pdf:sdk] returncode={proc.returncode} " f"produced_files={[p.name for p in produced]}"
            )
            ...
            if proc.returncode == 0 and pdf_files:
                ...
                try:
                    ...
                    _log.warning(f"[convert_to_pdf:sdk] target copy failed ({e}); " f"using temp path: {produced_pdf}")
                    keep_out_dir = True
                    return str(produced_pdf)

            _log.warning(
                f"[convert_to_pdf:sdk] FAILED — " f"returncode={proc.returncode}, produced={[p.name for p in produced]}"
            )
            return None
...
class TableRefiner:
    ...
    def cropping(
        self, bbox: dict, i_page: int, clip_margin: float = 0.05, zoom: float = 2.0  # 기본적으로 5% 여백 추가 권장
    ):
...
"""
            ...
            "messages": [
                {"role": "system", "content": system_prompt},
                {
                    ...
                    "content": [
                        {
                            "type": "text",
                            "text": "이 이미지를 위 규칙에 맞춰 검색용 HTML table fragment로 변환해줘."
                            + reference_text,
                        },
                        {
                            "type": "image_url",
    ...
    def _normalize_sparse_form_table(self, html: str) -> str:
        ...
        parts.append(
            "    <tr>" + "".join(f"<th>{self._escape_html_text(cell)}</th>" for cell in header_cells) + "</tr>"
        )
        parts.append("  </thead>")
        parts.append("  <tbody>")
    ...
    async def refine_vectors(self, vectors):
        ...
        for item in results:
            try:
                ...
                self._debug(
                    "테이블 감지 중 오류 발생/해당 테이블을 Docling 테이블로 사용합니다: %s", item, level="warning"
                )
...
class llm_serving:
    ...
    def call_with_body(self, body: Optional[dict] = None, output_format: BaseModel = None):
        """
        LLM output의 모든 데이터를 내보냅니다.
        """
        if body is None:
    ...
    async def async_call_with_body(self, body: Optional[dict] = None, output_format: BaseModel = None):
        """
    ...
    def _generate_text_from_items_with_headers(
        ...
        for i, item in enumerate(items):
            ...
            if isinstance(item, TableItem):
                ...
                if (
                    _as_int_flag(_first_kwarg(kwargs, "img_desc", "image_description_on", default=0), 0) == 1
                    or _as_int_flag(
                        _first_kwarg(kwargs, "img_to_table", "img_detail", "enhanced_image_description_on", default=0),
                        0,
                    )
                    == 1
                ):
    ...
    def _split_document_by_tokens(self, doc_chunk: DocChunk, dl_doc: DoclingDocument, **kwargs) -> list[DocChunk]:
        ...
        def is_local_section_boundary(idx: int) -> bool:
            ...
            if idx > 0:
                ...
                if get_first_page(prev_item) == item_page and is_local_heading_like_text(prev_item):
                    return False
        ...
        def split_items_by_token_and_page_limits(items_group, item_token_counts, max_tokens, max_pages):
            ...
            for idx, group in enumerate(items_group):
                ...
                group_pages = {page for item, _, _ in group for page in get_item_pages(item)}
                candidate_pages = cur_pages | group_pages
                token_exceeded = max_tokens > 0 and cur_tokens > 0 and cur_tokens + group_tokens > max_tokens
                page_exceeded = max_pages > 0 and cur_pages and group_pages and page_span(candidate_pages) > max_pages
        ...
        while i < len(sections_with_text):
            ...
            page_split_needed = self.max_pages_per_chunk > 0 and get_page_span(items) > self.max_pages_per_chunk
        ...
        for i in range(len(sections_with_text) - 2, -1, -1):
            ...
            if 0 <= next_level < current_level:
                continue
            if self.max_pages_per_chunk > 0 and get_page_span(items + n_items) > self.max_pages_per_chunk:
                continue
        ...
        for text, items, header_infos, header_short_infos in sections_with_text:
            ...
            elif self.max_pages_per_chunk > 0 and test_page_span > self.max_pages_per_chunk and len(merged_texts) > 0:
                b_new_chunk = True
...
class HwpProcessor:
    ...
    def load_documents(self, file_path: str, **kwargs: dict) -> DoclingDocument:
        """SDK 백엔드를 통해 문서를 로드"""
        from docling.document_converter import HwpxFormatOption

        # 요청마다 독립적인 pipeline_options 생성 (공유 상태 변이 방지) --> save_images, dump_sdk_output
        pipeline_options = PipelineOptions()
        pipeline_options.save_images = _as_bool_flag(kwargs.get("save_images", True), True)

        use_hwp_sdk = _as_bool_flag(kwargs.get("use_hwp_sdk", True), True)
        pipeline_options.dump_sdk_output = (
            _as_bool_flag(kwargs.get("dump_sdk_output", False), False) if use_hwp_sdk else False
        )
        ...
        if use_hwp_sdk:
            ...
            converter = DocumentConverter(
                format_options={
                    InputFormat.HWP: HwpxFormatOption(
                        pipeline_options=pipeline_options, backend=GenosHwpDocumentBackend
                    ),
            ...
            converter = DocumentConverter(
                format_options={
                    InputFormat.HWP: HwpxFormatOption(pipeline_options=pipeline_options, backend=HwpDocumentBackend),
                    InputFormat.XML_HWPX: HwpxFormatOption(
                        pipeline_options=pipeline_options, backend=HwpxDocumentBackend
                    ),
                }
            )
...
def _collect_hwp_formulas(hwp_doc: DoclingDocument) -> List[_HwpFormula]:
    ...
    for item, _ in hwp_doc.iterate_items(included_content_layers={ContentLayer.BODY}):
        if not isinstance(item, TextItem):
            continue
        label = getattr(item, "label", None)
        is_formula = label == DocItemLabel.FORMULA or (isinstance(label, str) and label == "formula")
        if not is_formula:
            continue
        text = item.text or ""
...
def _enrich_vectors_with_hwp_formulas(
    ...
    def split_header_prefix(text: str) -> tuple[str, str]:
    ...
    for match in matches:
        ...
        vec = vectors[v_idx]
        text = vec.text or ""
        header_prefix, body_text = split_header_prefix(text)
        body_norm = _normalize_text(body_text)
        length_ratio = len(hwp.norm) / max(len(body_norm), 1)

        # 벡터 본문이 이미 수식 블록이면 인라인 임베딩 금지 (블록 교체만)
        is_formula_block = body_text.strip().startswith("$$")

        new_body = None
        span = None
        if not is_formula_block:
            # 인라인: 스팬 탐색 후 $...$ 삽입 (LaTeX stripped fallback 포함)
            span = _find_formula_span_in_text(body_text, hwp.text)
            if span is None:
                stripped = _strip_latex(hwp.text)
                if len(stripped) >= 3:
                    span = _find_formula_span_in_text(body_text, stripped)

        if span:
            s, e = span
            new_body = body_text[:s] + f"${hwp.text}$" + body_text[e:]
        elif is_formula_block:
            new_body = f"$$\n{hwp.text}\n$$"
        elif length_ratio >= _FORMULA_EMBED_RATIO and bool(re.search(r"GLYPH\w*|[\ue000-\uf8ff]", body_text)):
            # 스팬을 잡지 못했더라도 본문이 글리프 중심이면 HEADER는 보존하고 본문만 교체한다.
            new_body = f"$$\n{hwp.text}\n$$"
        # 스팬/수식블록/글리프 근거가 없으면 주변 문맥 손실 방지를 위해 건너뜀

        if new_body is not None:
            new_text = header_prefix + new_body
            vectors[v_idx] = vec.model_copy(
                update={
                    "text": new_text,
                    "n_char": len(new_text),
                    "n_word": len(new_text.split()),
                    "n_line": len(new_text.splitlines()),
                }
            )
            applied += 1
...
class DocumentProcessor:
    ...
    def __init__(self):
        ...
        ocr_lang = [lang.strip() for lang in os.getenv("OCR_LANG", "korean").split(",") if lang.strip()] or ["korean"]
        ocr_options = PaddleOcrOptions(
            force_full_page_ocr=False, lang=ocr_lang, ocr_endpoint=self.ocr_endpoint, text_score=0.3
        )
    ...
    def _create_converters(self):
        ...
        self.second_converter = DocumentConverter(
            format_options={
                InputFormat.PDF: PdfFormatOption(
                    pipeline_options=self.pipe_line_options, backend=DoclingParseV4DocumentBackend
                ),
            },
        )
    ...
    def _configure_picture_description(
        self, image_description_on: int, enhanced_image_description_on: int, file_path: str, **kwargs
    ) -> None:
        ...
        if enabled:
            ...
            prompt_template = (
                enhanced_image_description_prompt if enhanced_image_description_on == 1 else image_description_prompt
            )
    ...
    def restore_header_labels(self, before_doc: DoclingDocument, after_doc: DoclingDocument) -> DoclingDocument:
        ...
        for item, level in before_doc.iterate_items():
            ...
            if (
                ref
                and label in [DocItemLabel.TITLE, DocItemLabel.SECTION_HEADER]
                and not self._is_non_structural_header_text(text)
            ):
  ...
  "title" : "추출 또는 생성된 해당 문서의 제목"
    ...
    async def __call__(self, request: Request, file_path: str, **kwargs: dict):
        ...
        table_refine_on = _as_int_flag(
            _first_kwarg(kwargs, "table_external_vlm", "table_refine_on", default=0),
            0,
        )
        # vT5_2 publicly receives only:
        # outline, chunk_size, table_external_vlm, img_desc, img_to_table.
        # Keep these downstream knobs at defaults for now; future versions can expose them.
        postprocess_hwp_formulas = False
        force_ocr = 0
        doc_no = 0
        related_to = 0
        # Internal OCR switch:
        # - force_ocr=1: run OCR first.
        # - force_ocr=0: run the standard-style first pass, then allow glyph/text-quality OCR fallback.
        ...
        kwargs["img_to_table"] = enhanced_image_description_on
        kwargs["enhanced_image_description_on"] = enhanced_image_description_on
        kwargs["img_detail"] = enhanced_image_description_on
        kwargs["table_external_vlm"] = table_refine_on
        kwargs["table_refine_on"] = table_refine_on
        kwargs["table_fix"] = table_refine_on
        kwargs["formula"] = 1 if postprocess_hwp_formulas else 0
        kwargs["postprocess_hwp_formulas"] = kwargs["formula"]
        ...
        kwargs["doc_no"] = doc_no
        kwargs["related_to"] = related_to
        kwargs["relate_to"] = related_to
        ...
        _log.info(
            "Image Option: %s ToC Option: %s Metadata Option: %s Chunk Max Tokens: %s Chunk Max Pages: %s Section Boundary Option: %s Local Boundary Option: %s Table Refine Option: %s Formula Option: %s Glyph OCR Fallback Option: %s OCR Fallback Option: %s Force OCR: %s export_to_html: %s",
        ...
        # 수식 후처리
        if postprocess_hwp_formulas and original_ext in (".hwp", ".hwpx"):
            _log.info(
                "Processing Korean Document (%s) with Unified HwpProcessor: %s",
                original_ext,
                original_file_path,
            )
            try:
                hwp_processor = HwpProcessor()
                hwp_doc = hwp_processor.load_documents(original_file_path, **kwargs)
