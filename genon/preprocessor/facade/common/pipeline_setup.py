...
layout(genos_layout) 설정은 여기 넣지 않았다 — 네 사본의 기본값과 지원 키가 실제로
달라서(타임아웃 1200 vs 3600, DotsOCR fallback 키 유무 등) 먼저 어느 쪽이 옳은지
정해야 한다.
"""
...
def resolve_ocr_runtime(cfg: dict, ocr_cfg: dict, *, default_endpoint: str = "") -> OcrRuntime:
    ...
    셋 다 없을 때 쓸 값은 default_endpoint 로 호출부가 정한다 — facade 마다 다르다.
    """
    cfg = as_dict(cfg)
    ...
    endpoint = (
        ...
        or cfg.get("ocr_endpoint", default_endpoint)
    )
